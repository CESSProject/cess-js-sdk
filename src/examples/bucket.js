 ...
 * @Autor: cess lab
 */
const { Bucket, InitAPI, testnetConfig, wellKnownAcct } = require("../../");
...
async function main() {
  ...
  result = await oss.createBucket(mnemonic, addr, BUCKET_NAME);
  console.log(getDataIfOk(result), "\n");
  ...
  result = await oss.deleteBucket(mnemonic, addr, BUCKET_NAME);
  console.log(getDataIfOk(result), "\n");
